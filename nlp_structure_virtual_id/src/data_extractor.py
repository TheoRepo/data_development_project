...
class DataExtractor(BaseSparkConnector):
    ...
    def run(self, source_table, target_table, the_date, file_no):
        ...
        data = data.selectExpr("row_key", "mobile_id", "event_time", "app_name", "suspected_app_name",
                               ...
                               "rule_ner_entity_udf(msg, app_name, suspected_app_name) as entity",
                               "rule_ner_entity_type_udf(msg, app_name, suspected_app_name) as entity_type",
        ...
        data = data.filter("entity != ''") # 把UDF返回""的数据过滤掉
        # 数据条数大小来设置一下分区数据
        data.cache()
        cnt = data.count()
